...
delta = 1e-4 * box  # halt parameter
sensDist = 1.5  # sensing range
alpha = 0.7  # velocity parameter
...
for i in range(int(n_fault_free / 2), n_fault_free):
    p_fault_free[i] = Point(0.5 - rand_addon + rand_addon * random.random(),
                            -rand_addon / 2 + rand_addon * random.random())
    p[i] = p_fault_free[i]
    rob_fault_free.append(FaultFreeRobot(p_fault_free[i], sensDist, alpha, velMax, delta, fixNeighbors=fixNeighbor))
    rob_fault_free[i].findNeighbors(p)
...
while t<21:
    ...
    for i in range(0, n_fault_free):
        for neighbor in rob_fault_free[i].neighbors:
            plt.plot([p_fault_free[i].x, p[neighbor].x], [p_fault_free[i].y, p[neighbor].y], linewidth=0.1,
                     color='gray')
    ...
    plt.pause(1)
    plt.savefig('./result/resilienceSuperiority/%s%d.eps' % (method, t))
...
# plt.ylabel("Position (X)", fontsize=25)
# ax = plt.gca()
# start, end = ax.get_xlim()
...
# plt.pause(1)
# plt.savefig('./result/resilienceSuperiority/positionChangeX_%s.eps' % (method))
#
# plt.figure(figsize=(10, 7))
# plt.plot(position_y_along_time)
# plt.draw()
# plt.xlabel("Iteration", fontsize=25)
# plt.ylabel("Position (Y)", fontsize=25)
# ax = plt.gca()
# start, end = ax.get_xlim()
# ax.xaxis.set_ticks(np.arange(start + 0.5, end + 0.5, 1))
# # ax.set_xticks([0, 0.3, 0.4, 1.0, 1.5])
...
# plt.yticks(fontsize=20)
# plt.pause(1)
# plt.savefig('./result/resilienceSuperiority/positionChangeY_%s.eps' % (method))

plt.figure(figsize=(5, 5))
plt.plot(diff_nf_and_nf_approx)
plt.draw()
plt.xlabel("Iteration", fontsize=20)
# plt.ylabel(r'\tilde{n}_{f_i} - n_{f_i}', fontsize=20)
ax = plt.gca()
...
plt.tight_layout()
plt.savefig('./result/resilienceSuperiority/diff_nf_%s.eps' % (method))
#plt.show()
...
plt.yticks(fontsize=15)
plt.tight_layout()
plt.savefig('./result/resilienceSuperiority/positionChange_%s.eps' % (method))
#plt.show()
