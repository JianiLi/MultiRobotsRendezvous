...
n = 151  # number of robots 151 for center and 104 for tver
n_faulty = 101  # number of faulty robots  101 for center and 54 for tver
n_fault_free = 50  # number of fault-free robots
...
for i in range(0, n_faulty):
    p_faulty[i] = Point(0.95 + 0.001 * random.random(), 0.95 + 0.001 * random.random())
...
while t < 21:
    ...
    plt.pause(1)
    plt.savefig('./result/attackToOnePoint/%s%d.eps' % (method, t))
...
plt.tight_layout()
plt.savefig('./result/attackToOnePoint/diff_nf_%s.eps' % (method))
# plt.show()
...
plt.tight_layout()
plt.savefig('./result/attackToOnePoint/positionChange_%s.eps' % (method))
# plt.show()
